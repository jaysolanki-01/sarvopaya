...
const PROJECTS: Project[] = [
  ...
  },
  {
    ...
    image: "/images/work-kareliya-ig.jpg",
    instagramHandle: "kareliya_equipments_india",
  },
  {
    id: "luniva-jewels",
    title: "Luniva Jewels",
    ...
    image: "/images/work-luniva-ig.jpg",
    instagramHandle: "luniva_jewels_",
  },
];
...
function ProjectCard({ p, idx }: { p: Project; idx: number }) {
  ...
  return (
    ...
    >
      ...
      {p.image && p.instagramHandle ? (
        ...
        <>
          <div className="relative w-full overflow-hidden" style={{ height: 460 }}>
            ...
            <div className="absolute top-0 left-0 right-0 z-10 flex items-center gap-2 px-3 py-2.5 bg-white/95 backdrop-blur-sm border-b border-black/[0.07]">
              <div className="w-6 h-6 rounded-full bg-gradient-to-br from-yellow-400 via-pink-500 to-purple-600 flex-shrink-0" />
              <span className="text-[12px] font-semibold text-black/80 tracking-tight">@{p.instagramHandle}</span>
              <svg className="ml-auto w-4 h-4 text-black/40" viewBox="0 0 24 24" fill="currentColor">
          ...
          <div className="flex items-center justify-between gap-3 px-5 py-4">
            <div>
              <p className="text-[10px] font-semibold uppercase tracking-wider text-black/35">{p.industry}</p>
              <h3 className="font-heading text-lg font-bold text-black leading-tight mt-0.5">{p.title}</h3>
              <p className="text-xs text-black/50 mt-0.5 line-clamp-1">{p.whatWeDid}</p>
            </div>
            {p.link && (
              <a
                href={p.link}
                ...
                className="flex-shrink-0 inline-flex items-center gap-1.5 rounded-xl px-3 py-2 text-xs font-semibold bg-black text-white hover:bg-black/80 transition-colors"
              >
      ...
      {/* ── Card body (skipped for Instagram cards which have their own footer) ── */}
      {!p.instagramHandle && <div className="flex flex-1 flex-col p-5 sm:p-6">
